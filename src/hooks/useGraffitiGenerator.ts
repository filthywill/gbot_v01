...
export const useGraffitiGenerator = () => {
  ...
  const generateGraffiti = useCallback(async (text: string) => {
    ...
    try {
      ...
      if (!cleanText) throw new Error('Please enter at least one letter (a-z)');
      if (cleanText.replace(/\s/g, '').length > 18)
        throw new Error('Maximum 18 letters allowed (excluding spaces)');
      ...
      if (missingLetters.length > 0) {
        console.warn(`The following letters are not available: ${missingLetters.join(', ')}`);
      }
